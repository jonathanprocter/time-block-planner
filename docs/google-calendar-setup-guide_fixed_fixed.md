...
6. Search for and select the following scopes:
   - `https://developers.google.com/workspace/calendar/api/auth` - See all your calendars
   - `https://developers.google.com/workspace/calendar/api/auth` - View and edit events on all your calendars
7. Click **"Update"**
...
5. Click **"Add URI"** and add the following:
   ...
   - `http://127.0.0.1:3000` (alternative localhost and port)
   - Add your production domain if deploying (e.g., `https://yourdomain.com`)

**Authorized redirect URIs:**
6. **IMPORTANT:** For Google Identity Services (GIS) OAuth flow, redirect URIs are NOT required. Leave this section empty for this application. GIS uses JavaScript origins instead of redirect URIs.
...
- Incorrect discovery document URL
...
5. Ensure the discovery document URL is correct: `https://www.googleapis.com/discovery/v1/apis/calendar/v3/rest`

#### Issue: 404 Errors in Browser Console

**Symptoms:**
- Browser console shows 404 errors
- Errors related to font loading or Google API libraries
- Application may still work but with errors

**Possible Causes:**
- Incorrect URLs for Google Fonts
- Incorrect discovery document URL
- Incorrect Google Identity Services library URL

**Solutions:**
1. **Check Font URLs:** Ensure font preconnect URLs point to `https://fonts.googleapis.com` and `https://fonts.gstatic.com`, NOT Stack Overflow or other sites
2. **Check Discovery Document:** Verify the Calendar API discovery document URL is `https://www.googleapis.com/discovery/v1/apis/calendar/v3/rest`
3. **Check GIS Library:** Ensure Google Identity Services loads from `https://accounts.google.com/gsi/client`
4. **Clear Browser Cache:** After fixing URLs, clear cache and hard reload (Ctrl+Shift+R or Cmd+Shift+R)
5. **Check Network Tab:** Open DevTools Network tab to identify which resources are returning 404

**Fixed in Latest Version:**
- ✅ Discovery document URL corrected to proper endpoint
- ✅ Font URLs corrected to Google Fonts API
- ✅ All Google library URLs verified and corrected
...
**Error Message:** "redirect_uri_mismatch" or "Origin not allowed"

**Solutions:**
1. **IMPORTANT:** For Google Identity Services (GIS), you need **Authorized JavaScript origins**, NOT redirect URIs
2. Ensure the port number matches (e.g., if using port 8000, it must be in authorized JavaScript origins)
3. Check for typos in the authorized JavaScript origins
4. Make sure you're using `http://` not `https://` for localhost
5. Add both `http://localhost:8000` and `http://127.0.0.1:8000` to cover all cases
6. After updating origins in Google Cloud Console, wait a few minutes for changes to propagate
...
```javascript
const SCOPES = 'https://developers.google.com/workspace/calendar/api/auth https://developers.google.com/workspace/calendar/api/auth';
```
...
- [Google API Explorer](https://developers.google.com/calendar/api/v3/reference/events/list) - Test API calls
...
3. Search [Stack Overflow](https://stackoverflow.com/questions/tagged/google-calendar-api) for similar issues
4. Check [Google's Issue Tracker](https://issuetracker.google.com/issues?q=componentid:190851)
...
This integration uses Google Calendar API and is subject to [Google's Terms of Service](https://developers.google.com/terms) and [Google API Services User Data Policy](https://developers.google.com/terms/api-services-user-data-policy).
